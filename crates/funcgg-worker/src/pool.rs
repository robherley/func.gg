...
impl Pool {
            ...
            loop {
                tokio::select! {
                    change = receiver.recv() => {
                        tracing::info!("Supervisor received: {:?}", change);
                        match change {
                            ...
                            Some(StateChange::Finished(worker_id, request_id)) => {
                                let mut current_workers = current_workers.lock().await;
    ...
    async fn next_worker_idx(&self) -> usize {
        ...
        if current_workers.len() >= self.pool_size {
            // our throughput sucks?
            tracing::warn!(
                active_workers = current_workers.len() + 1,
                available_workers = self.pool_size,
                "queue backup detected: more working than available"
            );
        }
